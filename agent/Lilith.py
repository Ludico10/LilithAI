...
class Lilith:
    def __init__(self):
        value_model_fn = lambda nS, nA: FCDuelingQ(nS, nA, hidden_dims=(128, 32))
    ...
    def train(self, env):
        final_eval_score = self.agent.train(gamma=1, max_episodes=1000, env=env)
        return final_eval_score

    def demonstration(self, env):
        result, history = self.agent.evaluate_once(self.agent.online_model, env)
        return history
