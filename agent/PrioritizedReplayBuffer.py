...
class PrioritizedReplayBuffer:
    ...
    def sample(self, batch_size=None):
        ...
        states_stack = np.vstack([sample[0] for sample in samples[:, self.sample_index]])
        actions_stack = np.vstack([sample[1] for sample in samples[:, self.sample_index]])
        new_states_stack = np.vstack([sample[2] for sample in samples[:, self.sample_index]])
        rewords_stack = np.vstack([sample[3] for sample in samples[:, self.sample_index]])
        is_terminal_stack = np.vstack([sample[4] for sample in samples[:, self.sample_index]])
        idxs_stack = np.vstack(idxs)
        weights_stack = np.vstack(normalized_weights[idxs])
        return idxs_stack, weights_stack, (states_stack, actions_stack, new_states_stack, rewords_stack, is_terminal_stack)
